...
在 Paseo 里看到 Pi 会话的**累计** token 用量、花费和上下文占用 —— 也就是 Pi TUI 状态栏（footer）那一行信息。Paseo 原生只在 agent 快照里暴露 `lastUsage`（单轮），所以整个会话的总量看不到。
...
- 服务端：`persistence` 解析结果缓存 120s（未找到 session 时只缓存 4s，因此新会话第一轮消息后就能显示），每个 session 文件只读增量，reader 数量上限 200（LRU），调用明细内存中保留最近 400 条（累计总量不受影响）。
...
- 用 Pi 自己的 `dist/core/usage-totals.js` 作为独立 oracle，对 5 个真实 session 文件（最大 2.3MB / 474 次调用 / 6160 万 token）逐字段比对 input / output / cacheRead / cacheWrite / cost，全部一致。
